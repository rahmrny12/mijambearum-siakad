                    ...
                    <div class="card-body box-profile">
                        <div class="text-center">
                            ...
                            @elseif (Auth::user()->role == 'Siswa')
                                <a href="{{ asset(Auth::user()->siswa(Auth::user()->no_induk)->foto) }}"
                        ...
                        @if (Auth::user()->role == 'Guru')
                            <ul class="list-group list-group-unbordered mb-3">
                                ...
                                <li class="list-group-item">
                                    <b>NIP</b> <a
                                        class="float-right">{{ Auth::user()->guru(Auth::user()->id_card)->nip }}</a>
                                </li>
                            </ul>
                        @elseif (Auth::user()->role == 'Siswa')
                            <ul class="list-group list-group-unbordered mb-3">
                                ...
                                <li class="list-group-item">
                                    <b>NIS</b> <a
                                        class="float-right">{{ Auth::user()->siswa(Auth::user()->no_induk)->nis }}</a>
                                </li>
                            </ul>
                        @else
                        @endif
                ...
                <div class="card card-primary">
                    ...
                    <div class="card-body">
                        ...
                        @if (Auth::user()->role == 'Guru')
                            ...
                            <p class="text-muted">
                                @php
                                    $mapelNames = Auth::user()
                                        ->guru(Auth::user()->id_card)
                                        ->mapel->pluck('nama_mapel')
                                        ->join(', ');
                                @endphp
                                {{ $mapelNames }}
                            </p>
                        ...
                        @if (Auth::user()->role == 'Guru')
                            ...
                            <p class="text-muted">
                                {{ date('l, d F Y', strtotime(Auth::user()->guru(Auth::user()->id_card)->tgl_lahir)) }}</p>
                            <hr>
                        @elseif (Auth::user()->role == 'Siswa')
                            <strong><i class="far fa-calendar mr-1"></i> Tanggal Lahir</strong>
                            <p class="text-muted">
