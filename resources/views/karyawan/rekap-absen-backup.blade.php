            ...
            <div class="card-body">
                <div class="d-flex justify-content-end">
                    <div class="col-md-4 mb-3 px-0">
                        <select id="status-absensi" class="form-control">
                ...
                <table id="example1" class="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        {{-- @foreach ($absen_karyawan as $data)
                            ...
                            <div class="modal fade" id="confirmAbsen" tabindex="-1" role="dialog"
                                    ...
                                    <form method="post" action="{{ route('absensikaryawan.confirm', $data->id) }}">
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Yakin ingin mengonfirmasi
                                                    absensi?</h5>
                                            </div>
                            ...
                            <div class="modal fade" id="rejectAbsen" tabindex="-1" role="dialog"
                                aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <form method="post" action="{{ route('absensikaryawan.reject', $data->id) }}">
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Yakin ingin menolak absensi?
                                                </h5>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary"
...
@section('script')
    <script>
        ...
        $("#status-absensi").on('change', function() {
            getAllAbsen($(this).val());
        })

        function getAllAbsen(status) {
            $.ajax({
                type: "GET",
                data: "user_id={{ $user_id }}&status=" + status,
                dataType: "JSON",
                url: "{{ route('absensikaryawan.all') }}",
                    ...
                    if (result) {
                        ...
                        $.each(result, function(index, val) {
                            ...
                            <tr>
                                ...
                                <td>${val.absen_karyawan.hari.nama_hari}</td>
                                <td>${new Date(val.created_at).toLocaleTimeString('en-US')}</td>
                                <td>${val.keterangan}</td>
                                <td>
                                    <img src="${assetBaseUrl + val.foto_awal}" id="preview" style="width: 100px"
                                        alt="Foto Kegiatan">
                                </td>
                                <td>
                                    <img src="${assetBaseUrl + val.foto_akhir}" id="preview" style="width: 100px"
                                        alt="Foto Kegiatan">
                                </td>
                                <td>
                                    ${val.user_id == auth_user_id
                                    ? val.status == 'dikonfirmasi'
                                        ? '<div class="badge badge-success p-2">Dikonfirmasi</div>'
                                        : val.status == 'ditolak'
                                        ? '<div class="badge badge-danger p-2">Ditolak</div>'
                                        : '<div class="badge badge-warning p-2">Proses</div>'
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary"
                                                    data-dismiss="modal">Batal</button>
                                                <a href="{{ url('karyawan/rekap/absen/reject') }}/${val.id}" class="btn btn-primary">
                                                &nbsp; Konfirmasi</a>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary"
                                                    data-dismiss="modal">Batal</button>
                                                <a href="{{ url('karyawan/rekap/absen/reject') }}/${val.id}" class="btn btn-danger">
                                                    &nbsp; Tolak</a>
                                            </div>
                                        </div>
                                    </div>
